...
/** 是否开启AI功能 */
@property (nonatomic, assign) BOOL openAI;

/**
 当前用户id
 会受FATStoreConfig中的encryptUserId影响，在网络接口中加密
 小程序(storage)缓存信息、授权信息、运行过程中产生的临时文件会存储在以userId命名的不同目录下。
...
/**
 ...
 可以是自己系统的userId、手机号、邮箱，会受FATStoreConfig中的encryptUserId影响，在网络接口中加密。
 对应管理后台的用户管理->成员管理->用户id。
 该属性影响打开体验版、审核版、开发版小程序功能；小程序用户灰度功能。
 注意：打开体验版、审核版、开发版时，会优先取memberUserId，如果没设置则取currentUserId。
 */
...
//是否允许截屏，默认为YES，允许截屏 更改后之前打开的页面不会生效
@property (nonatomic, assign) BOOL enableScreenShot NS_AVAILABLE_IOS(13_0);

/**
 截屏优先级设置，默认全局配置优先，允许截屏
 */
@property (nonatomic, assign) FATConfigPriority screenShotPriority;
...
/**
 SDK内所有的网络接口都需要将customPlatformHeader的内容添加至header中，再发起网络请求。
 注意，如果SDK调用的接口也传递了相同的key，则以SDK的为准
 */
@property (nonatomic, strong) NSDictionary *customPlatformHeader;
 ...
 */
@property (nonatomic, copy) NSString *pickerRegionPath;


/// 权限说明信息未配置时的策略，详细说明可以查看FATAuthDescStrategy
/// 如果小程序中的app.json 有配置 permission 权限说明，则 permission 的权限说明的优先权最高。不论authDescStrategy怎么设置，优先权都没 permission 高。
