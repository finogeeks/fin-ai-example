...
/**
 胶囊里的红点按钮的宽度，默认值为6
 */
@property (nonatomic, assign) CGFloat redDotBtnWidth;

/**
 胶囊里的红点按钮的高度，默认值为6
 */
@property (nonatomic, assign) CGFloat redDotBtnHeight;

/**
 胶囊里的红点按钮的右边距，默认值为6
 */
@property (nonatomic, assign) CGFloat redDotBtnRightMargin;

/**
 胶囊里的红点按钮的上边距，默认值为3
 */
@property (nonatomic, assign) CGFloat redDotBtnTopMargin;

/**
 胶囊里的红点按钮的深色图片对象，如果不传，会使用默认图标
 （明亮模式）
 */
@property (nonatomic, strong) UIImage *redDotBtnDarkImage;

/**
 胶囊里的红点按钮的浅色图片对象，如果不传，会使用默认图标
 （暗黑模式）
 */
@property (nonatomic, strong) UIImage *redDotBtnLightImage;
...
/**
 使用push动画打开小程序时，是否隐藏loading和首页的返回按钮
 */
@property (nonatomic, assign) BOOL hideLoadingAndHomeBackButtonWhenPushStyle;
...
///  获取小程序页面支持方向时是否忽略AppDelegate中application: supportedInterfaceOrientationsForWindow:返回的页面方向，默认为NO。
/// 小程序页面支持的方向会结合supportedInterfaceOrientationsForWindow(APP未实现则用工程方向)以及小程序页面配置的方向计算最终支持的方向，避免改变小程序页面方向可能导致的崩溃问题。
/// 为YES的话，会忽略supportedInterfaceOrientationsForWindow方法，直接用工程方向和小程序页面配置的方向计算最终支持的方向。(用于uni-app基座打包APP会崩溃的场景,supportedInterfaceOrientationsForWindow方法中会获取最顶层页面支持的方向)
@property (nonatomic, assign) BOOL ignoreAppDelegateSupportedInterfaceOrientations;
